 ...
 * @Description: ArraySet is a generic set data structure that is designed to be more memory efficient than a
 * traditional {@link java.util.HashSet}.  The design is very similar to
 * ArrayMap, with all of the caveats described there.  This implementation is
 * separate from ArrayMap, however, so the Object array contains only one item for each
 * entry in the set (instead of a pair for a mapping).
...
public final class ArraySet<E> implements Collection<E>, Set<E> {
    ...
    final boolean[] EMPTY_BOOLEANS = new boolean[0];
    final int[] EMPTY_INTS = new int[0];
    final long[] EMPTY_LONGS = new long[0];
