...
public class ShimmerFrameLayout extends FrameLayout {
  ...
  private static class Mask {
    ...
    public MaskAngle angle;//闪光的方向
    public float tilt;//闪光倾斜角度
    public float dropoff;
    public int fixedWidth;//闪光高亮的宽度，如果设置，则不管控件自身的宽度，默认值为0
    public int fixedHeight;//闪光高亮的高度，如果设置，则不管控件自身的高度，默认值为0
    public float intensity;
    public float relativeWidth;//闪光效果和控件宽之间的关联，默认值为1
    public float relativeHeight;//闪光效果和控件高之间的关联，默认值为1
    public MaskShape shape;//闪光的形状

    public int maskWidth(int width) {//真正的设置闪光的宽度，传入控件的宽度，如果没有设置固定宽度
  ...
  private static class MaskTranslation {//设置闪光效果的位移方向
  ...
  private Paint mAlphaPaint;//蒙版的透明度画笔
  private Paint mMaskPaint;//闪光效果画笔
  ...
  public ShimmerFrameLayout(Context context, AttributeSet attrs, int defStyle) {
    ...
    mMask = new Mask();//闪光信息封装类
    mAlphaPaint = new Paint();
    mMaskPaint = new Paint();
    mMaskPaint.setAntiAlias(true);//设置抗锯齿
    mMaskPaint.setDither(true);//抗抖动
    mMaskPaint.setFilterBitmap(true);//设置滤波
    mMaskPaint.setXfermode(DST_IN_PORTER_DUFF_XFERMODE);//设置图片相切模式
   ...
   */
  private void setMaskOffsetX(int maskOffsetX) {//位移X距离
   ...
   */
  private void setMaskOffsetY(int maskOffsetY) {//位移Y距离
  ...
  private Bitmap getMaskBitmap() {
    ...
    mMaskBitmap = createBitmapAndGcIfNecessary(width, height);//用控件的宽高创建一个bitmap
    Canvas canvas = new Canvas(mMaskBitmap);//用bitmap创建一个新的画布
    Shader gradient;//线性渐变
    switch (mMask.shape) {
      default:
      case LINEAR: {
        int x1, y1;
  ...
  private Animator getShimmerAnimation() {//获取闪光效果的animator
    if (mAnimator != null) {
      return mAnimator;
    }
