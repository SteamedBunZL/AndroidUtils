...
public final class LeakCanary {
  ...
  public static RefWatcher install(Application application,
    ...
    if (isInAnalyzerProcess(application)) {//判断是否是分析service 服务
      return RefWatcher.DISABLED;//如果是分析进程，就什么都不做，返回一个defalut的RefWatcher对象
    }
    enableDisplayLeakActivity(application);//设置系统组件是否可用，这里是设置DisplayActivity可用
    HeapDump.Listener heapDumpListener =
        new ServiceHeapDumpListener(application, listenerServiceClass);//初始化HeapDumpListener监听回调
    RefWatcher refWatcher = androidWatcher(application, heapDumpListener, excludedRefs);//工厂模式，生成RefWatcher对象
    ActivityRefWatcher.installOnIcsPlus(application, refWatcher);//注册ActivityRefWatcher的监听回调
    return refWatcher;//返回RefWatcher
  }
