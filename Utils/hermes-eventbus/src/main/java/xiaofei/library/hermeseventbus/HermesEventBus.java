...
public class HermesEventBus {
    ...
    public void init(Context context) {
        ...
        mMainProcess = isMainProcess(context.getApplicationContext());//判断是否是主进程
        if (mMainProcess) {//是主进程
            Hermes.init(context);//传入context
            Hermes.register(MainService.class);
            mMainApis = MainService.getInstance();//获取主进程消息分发实例-单例
        } else {//其他进程
